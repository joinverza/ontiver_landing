...
export function isPageScrollLocked() {
  ...
  return body.overflowY === "hidden" || body.overflowY === "clip" || body.position === "fixed"
...
export function scrollPageTo(
  ...
  if (target === null || (target instanceof HTMLElement && !target.isConnected) || isPageScrollLocked()) return;
  const complete = () => {
