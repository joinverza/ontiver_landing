...
import { joinWaitlist, sendContactRequest } from "../../../../shared/lib/landingApi";
...
const consentText =
  "I agree to join the Ontiver waitlist and have my signup details and interests sent to the Ontiver team for early-access follow-up.";
const fieldClass =
  "mt-2 min-h-12 w-full min-w-0 rounded-[10px] border border-[#b6caaa] bg-white px-4 text-body font-normal text-[#002d0e] outline-none placeholder:text-[#002d0e]/45 focus:border-[#007d21] focus:ring-2 focus:ring-[#007d21]/10 disabled:opacity-65";
...
const WaitlistForm = ({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const nextCompleted = {
      ...
      details: details.status === "fulfilled",
    };
  ...
  return (
    <form className={`text-left ${className}`} onSubmit={submit} aria-busy={state === "joining"}>
      <fieldset disabled={locked} className="grid min-w-0 gap-x-5 gap-y-4 sm:grid-cols-2">
        ...
        <label htmlFor={`${formId}-name`} className="text-body font-medium">
          Name *
          <input
            id={`${formId}-name`}
            name="name"
        ...
        <label htmlFor={`${formId}-email`} className="text-body font-medium">
          Email *
          <input
            id={`${formId}-email`}
            name="email"
        ...
        <label htmlFor={`${formId}-type`} className="text-body font-medium">
          User type *
          <select
            id={`${formId}-type`}
            name="userType"
            ...
            className={fieldClass}
          >
            {["Individual", "Business", "Developer", "Partner"].map((type) => (
              <option key={type}>{type}</option>
            ))}
          </select>
        </label>
        <label htmlFor={`${formId}-phone`} className="text-body font-medium">
          Phone <span className="font-normal text-[#526058]">(optional)</span>
          <input
            id={`${formId}-phone`}
            name="phone"
            type="tel"
        ...
        <label htmlFor={`${formId}-interest`} className="text-body font-medium sm:col-span-2">
          Interest area *
          <input
            id={`${formId}-interest`}
            name="interest"
            ...
            placeholder="For example, reusable identity for work or school"
            className={fieldClass}
          />
        </label>
        <div className="sm:col-span-2">
          <label
            ...
            className="flex items-start gap-3 text-meta leading-relaxed text-[#526058]"
          >
            <input
              id={`${formId}-consent`}
              name="consent"
              ...
              className="mt-1 size-5 shrink-0 accent-[#007d21]"
            />
      ...
      <p className="mt-3 text-meta text-[#526058]">
        Read our{" "}
        <Link to="/privacy" className="font-medium text-[#007d21] underline underline-offset-4">
          Privacy Policy
        </Link>
        .
      </p>
      ...
      >
        {state === "joining" ? (
          ...
          <>
            {partial ? "Retry remaining step" : submitLabel}
      ...
      <div aria-live="polite">
        {state === "joined" ? (
          <p className="mt-3 text-body text-[#007d21]">
            You're on the list! Your signup details have been sent to the Ontiver team.
          </p>
        ) : null}
      </div>
      {state === "error" && errorMessage ? (
        <p className="mt-3 text-body text-red-700" role="alert">
          {errorMessage}
        </p>
      ) : null}
    </form>
