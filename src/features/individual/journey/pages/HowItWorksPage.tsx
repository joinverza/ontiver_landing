...
const steps = [
  ...
  {
    title: "Invite someone to confirm",
    description:
      "Some requests need a referee, employer, or institution to confirm a claim — invite them directly from the app.",
  },
  ...
  {
    title: "Approve its next use",
    description: "Reuse the same proof for a future request, with your approval each time.",
  },
];
    ...
    <StandalonePage
      ...
      title={
        <>
          A request arrives.
          <br />
          <span className="text-[#007d21]">You stay in control.</span>
        </>
      }
    ...
    >
      <section className="section-space bg-[#f5f6f3]">
        <div className="site-container">
          <h2 className="section-heading max-w-[760px]" data-scroll-reveal>
          ...
          <div className="mt-10 grid items-start gap-10 lg:mt-14 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
            ...
            <ol className="divide-y divide-[#d5dfd2]">
              {steps.map((step, index) => (
                <li key={step.title} data-scroll-reveal>
                  <details open={index === 0} className="py-4">
                    <summary className="cursor-pointer text-card-title font-medium">
                      <span className="mr-3 text-sm text-[#007d21]">
