...
import { contactSocialIcons } from "../../contact/data/contact";
...
const ArticleSidebar = ({
  ...
  return (
    <aside className="min-w-0 lg:sticky lg:top-32 lg:self-start">
      <form
        className="rounded-2xl bg-[#f5f6f3] p-6 sm:p-8 lg:p-6"
        onSubmit={(event) => {
        ...
        <div className="mt-6 space-y-4">
          <div>
            <label
              ...
              className="mb-2 block text-sm font-medium text-[#002d0e]"
            >
              Email address
            </label>
            <input
          ...
          <label className="flex items-start gap-3 text-meta leading-relaxed text-[#002d0e]/65">
            <input
              type="checkbox"
              className="mt-0.5 h-4 w-4 shrink-0 accent-[#009311]"
              checked={consent}
              required
          ...
          >
            {subscribeState === "loading" ? (
              ...
              <>
                <Check size={17} aria-hidden="true" /> Subscribed!
              </>
            ) : (
              <>
                Subscribe <ArrowUpRight size={17} aria-hidden="true" />
              </>
            )}
          </button>
          {subscribeState === "done" && (
            <p role="status" className="text-meta text-[#007d21]">
              You are subscribed. Thanks for joining us.
            </p>
          )}
          {emailError && (
            <p id="article-newsletter-error" role="alert" className="text-meta text-red-700">
              {subscribeState === "error"
                ? "Subscription failed. Please try again."
                : "Enter a valid email and choose to receive updates to subscribe."}
            </p>
          )}
        </div>
      </form>
      <div className="mt-8 border-t border-[#dde6dc] pt-7">
        ...
        <div className="mt-5 flex gap-3">
          {contactSocialIcons.map(({ icon, ...platform }) => (
            <PlatformIcon key={platform.label} {...platform}>
