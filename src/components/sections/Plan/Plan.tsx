...
import { planInitialToggles, planInitialValues, planInputFields, planToggleItems, type PlanFieldKey, type PlanToggleKey, type PlanRecommendation } from "../../../data/plan";
...
type PlanProps = { billingCycle?: BillingCycle; onViewPlan?: () => void; onComparePlans?: () => void };

export default function Plan({ billingCycle = "monthly", onViewPlan, onComparePlans }: PlanProps) {
  const id = useId();
  ...
  const viewPlan = () => {
    const card = recommendation?.plan ? document.querySelector<HTMLElement>(`[data-plan="${recommendation.plan.toLowerCase()}"]`) : null;
  ...
  return (
    <section className="section-space bg-[#f7f8f5] text-[#002d0e]">
      <div className="site-container">
        <div data-scroll-reveal className="mb-12 max-w-[780px]"><p className="eyebrow">Explore your plan options</p><h2 className="mt-5 text-section font-normal">A plan that fits the way you grow.</h2></div>
        <div className="grid gap-6 lg:grid-cols-2">
          <form className="rounded-2xl border border-[#dde6dc] bg-white p-6 sm:p-9" onSubmit={(event) => { event.preventDefault(); checkBestPlan(); }}>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row"><button type="submit" className="button-primary grow">Explore plan fit<ArrowUpRight size={18} /></button><button type="button" className="button-secondary" onClick={() => { setValues(planInitialValues); setToggles(planInitialToggles); setRecommendation(null); }}><RotateCcw size={16} />Clear</button></div>
          </form>
          <PlanOrbitPanel billingCycle={billingCycle} monthlyVerifications={values.verifications ? Number(values.verifications) : undefined} recommendation={recommendation} onViewPlan={viewPlan} onComparePlans={onComparePlans ?? (() => scrollPageTo(document.getElementById("plan-comparison")))} />
        </div>
      </div>
    </section>
  );
